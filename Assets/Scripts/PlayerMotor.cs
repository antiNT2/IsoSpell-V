...
public class PlayerMotor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        #region Swing
        if (isSwinging && hasSetCollisionException == false)
        {
            Physics2D.IgnoreLayerCollision(this.gameObject.layer, LayerMask.NameToLayer("OneWayPlatform"), true);
            hasSetCollisionException = true;
        }
        else if (isSwinging == false && hasSetCollisionException == true)
        {
            Physics2D.IgnoreLayerCollision(this.gameObject.layer, LayerMask.NameToLayer("OneWayPlatform"), false);
            hasSetCollisionException = false;
        }
        #endregion
    ...
    public void MoveRight()
    {
        ...
        if (wallSliding || wallJumped || blockRightMovement || isBeingKnockbacked || startDash || isSwinging)
            return;
    ...
    public void MoveLeft()
    {
        ...
        if (wallSliding || wallJumped || blockLeftMovement || isBeingKnockbacked || startDash || isSwinging)
            return;
    ...
    public void Swing(float horizontalInput, float swingForceMultiplier = 1f)
    {
        PlayWalkAnim();
        return;
    }
